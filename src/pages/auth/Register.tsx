...
import { Loader2, Phone, Lock, User, Eye, EyeOff, HeartHandshake, Search } from 'lucide-react';
import { useAuth } from '../../components/AuthContext';
import { ApiError } from '../../lib/api';
import { cn } from '../../lib/utils';

type Role = 'careseeker' | 'caregiver';

const ROLES: { id: Role; label: string; description: string; Icon: typeof Search }[] = [
  {
    id: 'careseeker',
    label: 'Care Seeker',
  ...
  },
  {
    id: 'caregiver',
    label: 'Caregiver',
    description: 'Offer caregiving services',
    Icon: HeartHandshake,
  },
];
...
export const Register = () => {
  ...
  const [role, setRole] = useState<Role>('careseeker');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await signup({ name: name.trim(), phone: phone.trim(), password, role });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface-container-lowest rounded-3xl border border-outline-variant/15 shadow-sm p-8 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Role selector */}
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-on-surface">I am a…</label>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      role === id
                        ...
                        : 'border-outline-variant/40 bg-surface-container text-on-surface-variant hover:border-primary/40 hover:bg-surface-container-low'
                    )}
                  >
                    <Icon size={22} />
                    <div>
